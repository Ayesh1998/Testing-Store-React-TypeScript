...
export interface DemoProductsAllCategoriesProps {}

const DemoProductsAllCategories: React.SFC<DemoProductsAllCategoriesProps> = (
  props
) => {
  const [categoriesstate, setCategoriesstate] = useState(categories);

  const activeFunction = (ids: number) => {
    console.log("active function");
    var activePrevious = categoriesstate.find((item) => item.active === 1);
    var activePreviousTemp: any = {
      ...activePrevious,
      active: 0,
    };
